...
void HAL_CAN_RxCpltCallback(CAN_HandleTypeDef *hcan)
{
  ...
  if(hCAN.pRxMsg->DLC==8)
  {
    ...
    enstateallr=((((((enstateallr&0)|MOTOR5.enstate)<<1)|MOTOR6.enstate)<<1)|MOTOR7.enstate)<<1|MOTOR8.enstate;
    enstateallm=((((((enstateallm&0)|MOTOR1.enstate)<<1)|MOTOR2.enstate)<<1)|MOTOR3.enstate)<<1|MOTOR4.enstate;
        ...
        if(Rx_Buf[2]==RxCount-5)
        {
          switch(modbus_Cmd_Temp)
          {
            ...
            case BAT_READ_VOLTAGE:
              ...
              bat_state.voltage=(Rx_Buf[3]|Rx_Buf[4]<<8);
              //printf("[3] %x, [4] %x\n", Rx_Buf[3], Rx_Buf[4]);
              break;
            case BAT_READ_CURRENT:
